...
export default function Home() {
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
      ...
      <section className="hero-shell relative overflow-hidden border-b border-line bg-gradient-to-b from-background via-background to-panel">
        <div className="shell grid min-h-[660px] items-center gap-12 py-20 lg:grid-cols-[1.2fr_.8fr] lg:py-24">
          <div className="relative z-10 slide-in">
            <p className="eyebrow">Global Hardware and Prefab Pvt. Ltd. — Lalitpur, Nepal</p>
            <h1 className="mt-6 max-w-4xl font-heading text-[clamp(2.65rem,7vw,5.6rem)] font-semibold leading-[.96] tracking-[-.055em]">UPVC windows, doors and prefab homes, <span className="bg-gradient-to-r from-accent to-accent-deep bg-clip-text text-transparent">built for Nepal&apos;s climate.</span></h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-ink-soft">From first measurement to final fitting, we make practical building systems for homes, offices and project sites.</p>
            <div className="mt-9 flex flex-wrap gap-3"><a href="#contact" className="button button-primary focus-ring">Request a Site Visit</a><a href="#products" className="button button-outline focus-ring">Explore Products <span aria-hidden="true">→</span></a></div>
          </div>
          <div className="hero-window relative mx-auto aspect-square w-full max-w-[440px]" aria-label="Four-pane window illustration" role="img">
            <div className="absolute -left-8 -top-8 h-16 w-16 border-l-2 border-t-2 border-brass opacity-60" aria-hidden="true" />
            <WindowMark className="h-full w-full" />
            <p className="absolute -bottom-10 right-0 font-heading text-xs font-bold uppercase tracking-[.2em] text-ink-soft">Built to fit · Built to last</p>
          </div>
        </div>
      </section>
      ...
      <section id="products" className="section-space scroll-mt-24"><div className="shell"><SectionHead kicker="What we make" title="One team for the whole opening—and the whole structure." /><div className="mt-12 grid border-l border-t border-line sm:grid-cols-2 lg:grid-cols-3">{products.map((product, index) => <a className={`product-card group focus-ring ${index % 3 === 1 ? "hover:bg-sky-soft/45" : index % 3 === 2 ? "hover:bg-mint/60" : "hover:bg-background"}`} href={`/products/${product.slug}`} key={product.slug}><span className={`inline-flex h-12 w-12 items-center justify-center ${index % 3 === 1 ? "bg-sky-soft text-accent-deep" : index % 3 === 2 ? "bg-mint text-accent-deep" : "bg-[#FFF0E4] text-coral"}`}><LineIcon name={product.icon} /></span><h3 className="mt-10 font-heading text-xl font-semibold">{product.title}</h3><p className="mt-3 leading-7 text-ink-soft">{product.shortDescription}</p><span className="mt-7 inline-flex items-center gap-2 font-heading text-sm font-bold text-accent-deep">View details <span className="transition-transform group-hover:translate-x-1" aria-hidden="true">→</span></span><span className="corner-joint" aria-hidden="true" /></a>)}</div></div></section>
      ...
      <section id="gallery" className="section-space scroll-mt-24 border-y border-line bg-panel"><div className="shell"><div className="flex flex-wrap items-end justify-between gap-5"><SectionHead kicker="What we deliver" title="From accurate fitting to finished structures." /><p className="max-w-sm text-sm leading-6 text-ink-soft">Service visuals show the type of work we undertake. Actual Global UPVC project photographs can replace them as the portfolio grows.</p></div><div className="mt-12 grid auto-rows-[250px] gap-3 md:grid-cols-3">{galleryPhotos.map((photo) => <GalleryImage key={photo.src} {...photo} />)}<GalleryImage src={projectPhotos.home.src} alt="Completed residential home with white UPVC windows and doors" label="Your completed project" /></div><p className="mt-4 text-xs text-ink-soft">Illustrative service imagery. It does not depict completed Global UPVC client projects.</p></div></section>

      <section className="section-space border-b border-line"><div className="shell grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><SectionHead kicker="Project record" title="See the work, materials and care behind each installation." /><p className="mt-5 leading-7 text-ink-soft">We are building a verified project portfolio. Ask our team for recent examples relevant to your window, door or prefab requirements.</p><a href="tel:+9779803058200" className="button button-outline mt-6 focus-ring">Ask for project references</a></div><div className="grid gap-4 sm:grid-cols-3">{["Before installation", "During fitting", "Finished project"].map((label) => <article key={label} className="grid min-h-44 place-items-center border border-dashed border-accent/40 bg-mint/25 p-5 text-center"><div><p className="font-heading text-lg font-semibold">{label}</p><p className="mt-2 text-sm text-ink-soft">Add a real, consented project photo here</p></div></article>)}</div></div></section>
